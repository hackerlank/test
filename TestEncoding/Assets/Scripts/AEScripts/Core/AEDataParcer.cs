...
public class AEDataParcer  {
	...
	public static AECompositionTemplate ParseComposition(XmlNode composition) {
		...
		foreach (XmlNode layerNode in layersNodes) {
            ...
            AEFrameTemplate lastFrameTmp = null;
            ...
            foreach (XmlNode frameNode in frameNodes)
            {
                ...
                bool bHasSame = false;
                int nIndex = -1;
                foreach (AEFrameTemplate existFrame in layer.frames)
                {
                    nIndex++;
                    if (frame.EqualLastFrame(existFrame))
                    {
                        bHasSame = true;
                        lastFrameTmp = existFrame;
                        break;
                    }
                }
                ...
                if (!bHasSame)
                {
                    //if (lastFrameTmp == null)
                    {
                        lastFrameTmp = frame;
                        //Debug.Log("Add New Frame " + layer.name + "    " + lastFrameTmp.index);
                        layer.addFrame(lastFrameTmp);
                        nIndex = layer.frames.Count - 1;
                    }
                }

                //Debug.Log("Index " + layer.name + "    " + lastFrameTmp.index + "   " + frame.index);
                layer.addFrameIndex(nIndex);
