    ...
    public class CEncrypt
    {
        ...
        public class DES_cblock
        {
            ...
            public void init()
            {
                ...
                byte[] deskey = new byte[] { 41, 253, 1, 56, 52, 62, 176, 42 };
                for (int i = 0; i < 8; i++)
                {
                    arrBlock[i] = deskey[i];
                }
       ...
       public void ResetKey(byte[] key, byte index)
        ...
        public static void DES_set_key(DES_cblock key, DES_key_schedule schedule)
        {
            ...
            * for the inspiration. :-) */
            ...
            for (int i = 0; i < ITERATIONS; i++)
            {
                ...
                schedule.arrKeys[2*i+0] = func_ROTATE(ref t2, 30) & 0xffffffff;

                t2 = ((s >> 16) | (t & 0xffff0000));
                schedule.arrKeys[2 * i + 1] = func_ROTATE(ref t2, 26) & 0xffffffff;
            }
        }
        ...
        public void encdec_des(byte[] data, int index, int nLen, bool enc)
        {
            ...
            while (offset<=nLen-8)
            {
                ...
                if (enc)
                {
                    bEncrypt = LoginNetWork.IsNeedEncrypt(USAGE.eSend);
                    LoginNetWork.IncrementSendData();
                }
                else
                {
                    bEncrypt = LoginNetWork.IsNeedEncrypt(USAGE.eRecv);
                    LoginNetWork.IncrementRecvData();
                }
                if (bEncrypt)
                {
                    //Util.Log("有字节加密offset: " + (index + offset));
                    //Debug.Log("有字节加密offset: " + (index + offset));
